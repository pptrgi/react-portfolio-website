...
function ProjectDetailPage() {
        ...
        {splitParagraphsArray.map((paragraph, index) => {
          return (
            <p key={index} className="mb-4">
              {paragraph.includes("main features") ? (
  ...
  return (
    <div className="container mt-[6rem] mb-[3rem]">
      <h2 className="text-h2FontSize text-titleColor font-mediumWeight text-center mb-[4rem]">
        ...
        <div className="flex gap-2 flex-wrap justify-start md:justify-center lg:px-[4rem]">
          {currentProject.techStack.map((tech, index) => {
            return (
              <div
                key={index}
                className="text-textColor text-smallFontSize px-[0.5rem] py-[0.125rem] border-[2px] border-textColor"
              >
                {tech}
              </div>
            );
                    ...
                    <div key={index}>
                      <li
                        className={`${
                          ...
                          feature === "FRONTEND" ||
                          feature === "OTHERS"
                            ? "list-none font-semiBolded mt-[0.75rem]"
                            : ""
                        }`}
                      >
                        {feature}
      ...
      <div className="flex items-center mt-[4rem]">
        <div className="flex justify-start ">
          <Link
            to="/"
            className="flex gap-[5px] group items-center font-mediumWeight text-normalFontSize text-titleColor sm:gap-2"
          >
            <span className="group-hover:-translate-x-1 group-hover:text-black">
              <BiLeftArrowAlt />
            </span>
        ...
        <div className="flex justify-center items-center w-screen">
          <div className="flex justify-center gap-4 sm:gap-6 w-full">
            <a
              ...
              className="flex gap-2 items-center bg-titleColor px-[1.25rem] py-[0.5rem] rounded-md text-bodyColor font-mediumWeight text-smallFontSize hover:bg-black"
            >
              <span className="bg-transparent">Visit</span>
              <span className="bg-transparent">
                <FaExternalLinkAlt className="bg-transparent" />
              </span>
            </a>
            <a
              href={currentProject.gitHub}
              className=" bg-titleColor px-[1.25rem] py-[0.5rem] rounded-md text-bodyColor font-mediumWeight text-smallFontSize hover:bg-black"
            >
              GitHub
            </a>
          </div>
