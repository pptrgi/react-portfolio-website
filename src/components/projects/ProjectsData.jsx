...
export const serveProjects = () => {
  const projectsInfo = [
    ...
    },
    {
      ...
      description:
        "NyumbaHub is a vacant houses finding service based in Kenya. It includes a non-relational database MongoDB, Node/Express.js RESTful API hosted as a serverless function, and a React frontend where the house properties data is served to.\n\nThe API employs the MVC design pattern. Using mongoose ODM (Object-Data Modelling), there are defined schemas for the 4 models used throughout the application. Public routes are available to all users while private routes are protected with JSON Web Token (JWT) and either require signing in to access service or can only be accessed by privileged users. Images are uploaded to cloudinary with the help of multer and sharp. The backend accepts URL query from a request to query the database and return filtered and/or sorted response data. It is a serverless function and has a UI to help users efficiently navigate through it.\n\nThe application's frontend includes an aesthetic, functional and easy to use fully responsive UI that enhances UX while still putting the UI/UX design principles into consideration. With Redux and redux toolkit the application is able to manage the state effectively, thanks to createAsyncThunk() the frontend can make API calls and handle loading, success and error states in async operations. It has a number of navigation options including breadcrumbs, pagination, ham menu for mobile and header navigation. To enhance security, the frontend will privatize routes that need not be open.\n\nAlso included in the application is the well-outlined terms of service in the Terms of Service page and privacy policy in the Privacy Policy page.\n\nThe main features of the overall application are:",
      features: [
        "Backend".toUpperCase(),
        "Serverless",
